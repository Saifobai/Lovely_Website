...
export default function BookingCalendar({ activeService, isEmbedded }) {
  ...
  return (
    <div className="flex flex-col h-full min-h-[700px]">
      {/* BOARD HEADER */}
      <div className="p-8 border-b border-white/5 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h2 className="text-3xl font-black uppercase italic tracking-tighter">
            What is the{" "}
            <span className="text-blue-500">Best date for yoy?</span>
          </h2>
      ...
      <div className="flex-1 grid lg:grid-cols-12 overflow-hidden">
        ...
        <div className="lg:col-span-5 p-8 bg-white/[0.01]">
          ...
          {dayLabel && (
            <p className="text-[10px] text-blue-400/50 font-mono mb-6 uppercase tracking-widest">
              {dayLabel}
            </p>
          )}
      ...
      {/* THE BOARD FOOTER (The Input Bar) */}
      <div className="p-8 border-t border-white/5 bg-black/40">
        <div className="flex flex-col md:flex-row gap-4 items-center">
          <div className="relative flex-1 w-full">
            <input
              ...
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              placeholder="ENTER_ENCRYPTED_EMAIL_ADDRESS"
              className="w-full bg-white/5 border border-white/10 px-8 py-5 rounded-2xl font-mono text-xs focus:border-blue-500 transition-all outline-none"
            />
          </div>
          <button
            onClick={submitBooking}
            disabled={loading || !selectedTime || !email}
            className="w-full md:w-auto min-w-[300px] bg-blue-600 hover:bg-blue-700 text-white py-5 px-10 rounded-2xl font-black uppercase italic text-xs tracking-[0.2em] transition-all flex items-center justify-center gap-3 disabled:opacity-20 shadow-xl shadow-blue-900/20"
          >
            {loading ? (
              "Initializing_Protocol..."
            ) : (
              <>
                <Lock size={14} /> Secure Protocol Hold
              </>
            )}
          </button>
        </div>
