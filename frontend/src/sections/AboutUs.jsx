...
const TeamCard = ({ member, isActive }) => {
  return (
    ...
    >
      {/* Dossier ID Header */}
      <div className="absolute top-4 lg:top-6 left-4 lg:left-6 right-4 lg:right-6 flex justify-between z-30">
        <div
          className={`px-2 py-0.5 bg-[#0B1320]/80 border border-[#3B82F6]/20 rounded font-mono text-[8px] lg:text-[9px] uppercase tracking-widest text-[#3B82F6] transition-opacity duration-500 ${
            isActive ? "opacity-100" : "opacity-0"
          }`}
        >
          {member.position}
        </div>
        <span className="text-[8px] lg:text-[10px] font-mono text-[#4B5563] uppercase">
          ID_{member.name.split(" ")[0]}
        </span>
      </div>

      {/* Image Container */}
      <div className="relative aspect-[4/5] lg:aspect-[7/6] overflow-hidden border-b border-[#1F2937]">
        <img
          src={member.image}
      ...
      <div className="p-6 lg:p-10">
        <h4 className="text-xl lg:text-2xl font-black uppercase text-[#F5F7FA] tracking-tighter">
          {member.name}
        </h4>
        <p className="text-[10px] lg:text-[11px] font-mono text-[#3B82F6] mt-2 uppercase tracking-widest leading-relaxed">
          {member.role}
        </p>
      </div>
